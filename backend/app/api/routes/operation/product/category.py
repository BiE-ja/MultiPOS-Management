...
async def read(category_id:int, db: SessionDep, user : CurrentUserDep): 
    try:
...
async def update(category_id:int, category_updated : product_schema.ProductCategoryUpdate, db:SessionDep,user : CurrentUserDep): 
    try:
...
async def delete(area_id: int, category_id:int, session: SessionDep, user: CurrentUserDep):
