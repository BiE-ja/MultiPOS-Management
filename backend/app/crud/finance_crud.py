...
from app.models.finance.cash import CashTransaction, TransactionDirection, TransactionState
...
class FinanceManager:
    ...
    async def get_cash_register_transactions_by_date(self, cash_register_id: int, dateof: datetime) :
        """Get all transactions for a specific cash register on a given date"""
    ...
    async def calculate_theoretical_amount(self,cash_register_id: int, dateof : datetime):
        """Get the theoretical amount of cash in the register for a given date"""
        total = 0
        for trans in await self.get_cash_register_transactions_by_date(cash_register_id = cash_register_id, dateof = dateof):
    ...
    # The date is expected to be in the format of datetime.
    async def count_transactions(self, cash_register_id:int, dateof: datetime):
